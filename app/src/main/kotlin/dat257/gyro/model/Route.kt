...
data class Route(var coordinates: MutableList<Pair<String, Coordinate>>) : Jsonable {
    ...
    fun getDistance(): Double {
        ...
        for (i in 0 until coordinates.size - 1) {
            distanceToReturn += calcDistance(
                coordinates[i].second.latitude,
                coordinates[i].second.longitude,
                coordinates[i + 1].second.latitude,
                coordinates[i + 1].second.longitude
            )
        }
